...
func IReallyNeedToFindAWayToNameAnotherMain() {
	...
	// НЕ НУЖНО ЕЁ ИСПОЛЬЗОВАТЬ, она медленнее и менее безопа
	fmt.Println(s)
	...
	// срезы на основе среза дишь ссылаются на основной срез (bro wtf)
	...
	// Если не требуется кол-во скопированных переменных, то его можно не присваивать переменнной

	// x := []int{1,2,3,4}
	// num := copy(x[:3],x[1:])
	// В данном случае последние три элемента среза X скопируются в первые три элемента
	// этого среза
